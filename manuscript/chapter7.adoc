...
A kontext `OnConfiguring`-ját így **töröljük ki**!.

A kontext regisztrálása a `Program` osztály `WebApplicationBuilder`jébe:
...
Az EF naplózást az ASP.NET Core naplózó rendszere végzi, amit az alapértelmezett `Host` már inicializál, így ezzel kapcsolatban nincs teendőnk. Viszont egy új kontext konstruktorra lesz szükségünk, ami `DbContextOptions<AppDbContext>`-et vár.
...
.NET 6-ban ehhez szükségünk lesz egy `IDesignTimeDbContextFactory<TContext>` interfészt megvalósító osztályra is, amit vegyünk fel `AppDbContextDesignTimeFactory` néven a DAL projektbe. Az implementációban csak vissza kell adnunk egy DbContext példányt. Itt most nem számít a connection string, viszont gondoljunk arra, hogy ezt az osztályt fogja használni a migrációk futtatása során is a keretrendszer, így ide is érdemes megadni helyes connection string-et vagy a parancssorból argumentumként adjuk át a migráció futtatásakor. 
...
A _Controllers_ mappában hozzunk létre egy kontrollert (menu:Add[Controller > bal fában Common > API > jobb oldalon API Controller with actions using Entity Framework]). A felbukkanó ablakban válasszuk ki a `Product` entitás osztályt (azaz az Entities névtérbelit), a kontextet és adjunk meg nevet ( `EFProductsController`).

WARNING: Figyeljünk rá, hogy ne a `Dtos` névtérből adjuk meg a DTO típust a tényleges entitástípus helyett.

A legenerálódó kontroller már használható is. Állítsuk át a zöld nyíl mellett az indítási konfigurációt a projektnevesre, hogy ne IIS Express induljon és így lássuk a konzolon a naplót. Indítsuk a projektet és próbáljuk például lekérni az összes terméket az **api/efproducts** címről vagy a swagger felületről.
...
A `Program`-ban adjuk hozzá és konfiguráljuk a leképezési szolgáltatást.
...
Hogy ne zavarjon be a swaggernek az EFProductControllerben használt entitás osztályok, töröljük ki a Controllers mappából az EFProductControllert!
...
A `ProjectTo` metódust felfoghatjuk a továbbiakban egy LINQ-s `Select()` operátornak, annyi különbséggel, hogy az automapper generálja azt az expressiont, ami alapján előáll majd az eredmény.
...
Alternatíva lehetne még ennél a függvény esetében, hogy lekérdezzük ID alapján az entitást és AutoMapperrel a lekérdezett objektumba mappeljük a DTO-t. Ebben az esetben nincs szükség Attach-ra és állapotkezelésre sem.
...
Próbáljuk ki a műveletet Swaggerből. Egy `Product`-ot kell felküldenünk. erre egy példa érték:
...
WARNING: Ha Fiddlerből vagy Postmandből tesztelünk. ne felejtsük el a _Content-Type_ fejlécet  *application/json*-re állítani! Figyeljük meg a kapott választ. A válaszból másoljuk ki a _Location_ fejlécből az URL-t és hívjuk meg böngészőből.
...
Szokás szerint konfiguráljuk a `Program` osztályban. Sose adjuk vissza a kivétel részleteit (szigorú _exception shielding_), illetve a saját kivételtípusunkat képezzük le 404-es hibára.
