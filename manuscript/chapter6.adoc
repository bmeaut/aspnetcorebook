...
Az OR leképzés két fő részből áll, az egyik az adatbázis séma, a másik pedig egy menedzselt kódbéli objektum modell. Esetünkben a C# kódban lévő osztályokat képezzük le adatbázisbeli objektumokká, ezt hívjuk _Code-First_ mapping módszernek. A másik irány is lehetséges, ha már van egy adatbázis sémánk, akkor azt is leképezhetjük Code-First modellé. Ezt a folyamatot _Reverse Engineered Code-First_-nek hívjuk (ez utóbbival nem foglalkozunk ezen gyakorlat keretében).

=== A Code-first mapping módszer

A Code-First módszer lényege, hogy elsőként az OO entitásokat definiáljuk egyszerűen programkódban, majd a leképezést szintén programkódban. A leképezés alapján az EF eszközök képesek az adatbázis létrehozására, inicializálására és a séma változáskövetésére is (lásd _Code-First Migrations_).

=== Az entitások definiálása

Készítsünk egy konzolos alkalmazást, majd a projekten belül hozzunk létre egy _Entities_ nevű mappát. Adjunk hozzá a mappához egyszerű osztályokat az alábbi sémának megfelelően:
...
image::images/efcore-bridgetab.png[Product-Order kapcsolótábla]

Ezután így néznek ki az entitások:
...
include::code/efcore-productorder.cs[]
----

TIP: A fenti kódok kóddarabkákként is elérhetőek.

== Kapcsolat az adatbázissal

=== DbContext - NuGet

Az entitásokat definiáltuk, a mapping-et az EF eszére bíztuk, a következő lépés a DB séma létrehozása a mapping alapján, amit képes az EF létrehozni. Műveletet az ún. _kontext_-en keresztül tudunk végezni. Érdemes saját kontext típust létrehozni, amit az alap `DbContext`-ből származtatunk. Eddig még nem is írtunk semmilyen EF specifikus kódot, most viszont már kell a `DbContext` típus, így NuGet-ből hozzá kell adnunk a *Microsoft.EntityFrameworkCore.SqlServer* csomagot. Nem ez a csomag tartalmazza a `DbContext`-et, viszont függőségként hivatkozza (*Microsoft.EntityFrameworkCore*).
...
Az automatikusan létrejövő MSSQLLocalDB nevű LocalDB példány connection stringjét adjuk meg, pontosabban az _SQL Server Object Explorer_ ablak segítésével másoljuk ki: _(localdb)\MSSQLLocalDB_ szerveren jobbklikk → Properties → Connection String. A kimásolt stringben az _Initial Catalog_ értékét (a DB nevét) a *master*-ről változtassuk meg valamilyen más névre, például a neptun kódunkra.
...
TIP: A nagyobb rugalmasság érdekében érdemes a connection stringet konfigurációs fájlba helyezni, és egy saját `IDbContextFactory` implementációval, vagy az ASP.NET konfigurációs megoldásaival felolvasni. Erre egy későbbi gyakorlaton nézünk példát.
...
Hozzuk elő a *Package Manager Console*-t. (Tools → Library Package Manager). Ellenőrizzük, hogy a _Default Project_ legördülőben a mi projektünk van-e kiválasztva. Az `Add-Migration <név>` paranccsal tudunk készíteni egy új migrációs lépést, így az első migrációnk a kiinduló sémánk migrációját fogja tartalmazni.
...
WARNING: Bizonyos LocalDB verzióknál hibára futhat az adatbázislétrehozás (_CREATE FILE encountered operating system error 5(Access is denied.)_), mert rossz helyen próbálja létrehozni az adatbázisfájlt. Ilyenkor az SQL Server Object Explorer ablakban bontsuk ki a LocalDB példányunk, alatta a _Databases_ mappán jobbklikk → _Add New Database_. A megjelenő ablakban adjuk meg névként ugyanazt az adatbázisnevet, amit korábban a connection string-ben a master helyett megadtunk.

Ellenőrizzük le az adatbázis sémáját az _SQL Server Object Explorer_ ablakban. Mutassuk meg, hogy pusztán konvenciók alapján milyen tulajdonságokat talált ki az EF.
...
=== Mapping és egyéb metaadatok megadása II. -- fluent és attribute mapping konfiguráció
...
Ellenőrizzük, hogy a _Name_ oszlop most már az új kényszereknek megfelelően lett-e felvéve, és hogy a termékek tábla megfelelő néven jelenik-e meg. Ezzel kész a DAL rétegünk konfigurációja.
...
WARNING: fontos, hogy ne jöjjön létre külön logger példány minden context példányhoz, ezért lett a `MyLoggerFactory` statikus.

TIP: Ha nem a konzolt szeretnénk teleszemetelni, akkor akár a Debug outputra is írhatunk. Ehhez a _Microsoft.Extensions.Logging.Debug_ csomagban található `AddDebug` metódust kell használjuk.
...
TIP: a háttérben az EF minden új entitásnak kioszt egy átmeneti azonosítót, amit felhasználhatunk a fenti hiba elkerülésére, ha semmiképp sem akarjuk a navigációs property-ket használni. Így tudnánk a context-től elkérni: `context.Entry(cat_drink).Property(e => e.Id).CurrentValue`
...
TIP: Ez rámutat az EF egy nagy előnyére, bonyolult lekérdezéseket megírhatunk kisebb, egyszerűbb részletekben írhatunk meg, az EF pedig összevonja, sőt optimalizálhatja is a lekérdezést.
...
TIP: Itt is érdemes összevetni a `where` operátor definícióját (kurzorral ráállva kbd:[F12], vagy jobbklikk _Go To Definition_) a két lekérdezésrészben.
...
Ha a kapcsolódó `Order` listát is szeretnénk kitöltetni, akkor ott egyrészt a `ProductOrders` listát is be kell `Include`-olni, másrészt pedig még egy kapcsolattal továbbmenve a `ProductOrder` `Order` tulajdonságát is be kell töltetni. Az ilyen többszintes hivatkozást az `Include` és `ThenInclude` használatával lehet elérni:
...
TIP: Adatkezelő alkalmazásokban az adatbázisbeli törlés (SQL `DELETE` utasítás) helyett gyakran inkább logikai törlést alkalmaznak. A logikai törlés megvalósításával ezen gyakorlat keretében nem foglalkozunk.
