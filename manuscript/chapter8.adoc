...
* https://github.com/RicoSuter/NSwag/wiki/NSwagStudio[NSwag Studio] - itt is elég csak a legfrissebb zip verziót (például https://github.com/RicoSuter/NSwag/releases/download/v13.15.10/NSwag.zip[ezt]) az *Assets* részről letölteni.
...
Adjunk a solution-höz egy konzolos projektet (Console App (.NET 6), **nem .NET Framework!**) _WebApiLab.Client_ néven. Adjuk hozzá a projekthez a *System.Text.Json* Nuget csomagot.
A _Program.cs_-ben írjuk meg az egy terméket lekérdező függvényt (`GetProductAsync`) és hívjuk meg.
...
static async Task GetProductAsync(int id)
{
    ...
    var response = await client.GetAsync(new Uri($"http://localhost:5000/api/Product/{id}"));
...
Az OpenAPI leíró, illetve a dokumentációs felület kiszolgálására regisztráljunk egy-egy NSwag middleware-t az *Endpoint MW elé*. Az eddigi Swagger támogatással kapcsolatok kódot törölhetőek.
...
* Output rész (jobb oldal) - CSharp Client fül - Settings alfül: fölül a _Namespace_ mezőben adjunk meg egy névteret, pl. _WebApiLab.Client.Api_ (Többi beállítás maradhat alapértelmezetten)
