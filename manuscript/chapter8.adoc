...
TIP: A háttérben az EF update művelet során egy plusz feltételt csempész az UPDATE SQL utasításba, mégpedig, hogy a `RowVersion` mező adatbázisbeli értéke ugyanaz-e, mint az általa ismert (a kliens által látott) értéke. Ha ez a feltétel sérül, akkor konkurenciahelyzet áll fent, mivel valaki már megváltoztatta a DB-ben lévő értéket.
