...
[source,csharp]
----
var numbers = new Dictionary<int, inTIP: 
{
    [1] = 5,
    [2] = 4,
    [3] = 3,
...
Amennyiben egy metódus generátor (`IEnumerable` vagy `IEnumerable<TIP: ` visszatérési értékű), az egyes elemeken történő iteráció a generátorok egymásba ágyazását jelenti, azaz az egyes generátorokban a `yield return` által visszaadott értéket fogja az enumerátor `MoveNext` metódusa visszaadni. Amíg az `IEnumerable`-re van referenciánk, és nem járjuk azt közvetlenül be, addig _késleltetett kiértékelésről_ beszélünk.
...
[source,csharp]
----
public class RandomOrderedList<TIP:  : IEnumerable<TIP: 
{
    public RandomOrderedList(IList<TIP:  source) => Source = source;
    private IList<TIP:  Source { get; }
    public IEnumerator<TIP:  GetEnumerator() => new RandomEnumerator<TIP: (Source);
    IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();
}

public class RandomEnumerator<TIP:  : IEnumerator<TIP: 
{
    private IList<TIP:  Source { get; }
    private int[] indexes;
    private int currentIndex;
    public RandomEnumerator(IList<TIP:  source)
    {
        Source = source;
        Reset();
    }
...
[source,csharp]
----
foreach (var elem in new RandomOrderedList<inTIP: (Enumerable.Range(0, 100).ToList()))
